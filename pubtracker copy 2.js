...
async function handleClick() {
    ...
    slice.forEach(s => {
      ...
      for (const pub of pubs) {
        if (dist2(s.lat, s.lon, pub.lat, pub.lon) < radius2) {
          ...
          if (perYear[yr]) {
            perYear[yr].visits += 1;
            if (isFirstEver) perYear[yr].newPubs += 1;
          }
  ...
  const pct = ((totalVisited / pubs.length) * 100).toFixed(2);
  ...
  // build 5-year summary
  const summaryLines = Object.entries(perYear)
    .sort((a,b)=>a[0]-b[0])
    .map(([yr, stats]) =>
      `${yr}: ${stats.visits} visits, ${stats.newPubs} new pubs`
    );

  const txt =
    `📊 Percentage visited:\n` +
    `• ${totalVisited} of ${pubs.length} pubs = ${pct}%\n\n` +
